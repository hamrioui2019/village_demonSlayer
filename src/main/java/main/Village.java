...
//   Ajouter une liste de demonSlayerSedentaire pour nous permettre de récupérer leurs informations et les lier au village 
   ...
   private List<Ninja> listNinja;
    ...
    public Village()
    {
        ...
        this.listDemonSlayerSedentaires = new ArrayList<DemonSlayerSedentaire>();
     ...
     public void addAllianceVillage(Village nouvelAllie)
    {
    	 if (this.getAllies().contains(nouvelAllie))
    		 return;
    	 if (this.getEnnemis().contains(nouvelAllie))		//Si le nouvel allié est un ennemi, on le retire de la liste des ennemis et on le rajoute dans la liste des alliés
       ...
       public void declarerGuerre(Village nouvelEnemie)
    {
    	 if (this.getEnnemis().contains(nouvelEnemie))
      		 return;
      	 if (this.getAllies().contains(nouvelEnemie))		//Si le nouvel ennemi est un allié, on le retire de la liste des alliés et on le rajoute dans la liste des ennemis
    ...
    public void ajouterDemonSlayerSedentaire(DemonSlayerSedentaire demonslayer) {
    	if((demonslayer.getVillage() == null || demonslayer.getVillage() == this) && !(this.listDemonSlayerSedentaires.contains(demonslayer))) 
    	{
    		this.listDemonSlayerSedentaires.add(demonslayer);
        	demonslayer.setVillage(this);
        	this.nbrHabitants++;        	
    	}
    }
    
    public List<Ninja> getListNinja() {
		return Collections.unmodifiableList(this.listNinja);
	}
    
    public void ajouterNinja(Ninja ninja) {
    	if((ninja.getVillage() == null || ninja.getVillage() == this) && !(this.listNinja.contains(ninja))) 
    	{
    		this.listNinja.add(ninja);
